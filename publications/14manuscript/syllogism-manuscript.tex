...
In this paper, we aim to explain human reasoning performance on the syllogistic task by formalizing a number of intuitions. The first is that human reasoning is best characterized as reasoning under \emph{uncertainty}, and that this in turn is best described by the tools of probability theory. We formalize probabilistic syllogistic reasoning with a model that reasons over abstract \emph{situations}, consisting of objects with properties, which are in turn sampled stochastically. These abstract situations bear some resemblance to mental models on syllogisms \cite{JL1983}. We begin by comparing this model to two meta-analysis data sets and show an overall strong correspondence to the data.

The model we present is a Bayesian model and that means the prior distribution must be taken seriously. It's known that the content of syllogisms affect reasoning \cite{Wilkins28} but never before has a model been able to accommodate background knowledge without positing additional processing steps and/or some assumed categorical distinction between believable and unbelievable statements. Background knowledge fits naturally within the Bayesian model framework, as it simply describes an alternative prior distribution over situations. We test the influence of the prior distribution over situations (i.e. background knowledge) on conclusions drawn in a number of syllogisms where our model predicts differences. The model predicts subtle graded effects of background knowledge and the experiment confirms these predictions. 

In addition to the rich quantitative data that characterize the syllogistic reasoning task, a number of important qualitative phenomena have been observed. These cannot be understood by the semantics of the language used in syllogistic reasoning tasks alone. We go beyond natural language semantics, and consider the pragmatics of language understanding, the idea that people derive the meaning of words by a social reasoning process between agents. We formalize this intuition using recent advances in the field of probabilistic pragmatics. In this framework, the reasoner imagines the premises of the syllogism to come from an informative agent (``the experimenter''). This bears some similarity to previous approaches that examine the role of scalar implicature in syllogistic reasoning. Importantly, our model makes use of the implicit Question Under Discussion (QUD) in the syllogistic task, i.e. ``what is the relationship between the conclusion terms?'' and leads to interesting qualitative phenomena that differ from what one would expect from scalar implicature alone. In Experiment 2, we investigate a subset of syllogisms expected to have strong pragmatic effects. We show the pragmatic reasoning model provides the best fit to Experiment 2 data. We also show that the pragmatics model fits the Meta-analysis and Experiment 1 data even better than the original models.
...
We've seen so far that the content of the syllogism affects reasoning in a way perfectly compatible with the Bayesian notion of a prior distribution using our probabilistic model specification. The World-bound Bayesian model captures the modal responses as well as the graded quantitative data inherent in these syllogisms. 
