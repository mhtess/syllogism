...
The form of this exchange resembles a syllogism: a two- sentence argument used to relate two properties (or terms: A, C) via a middle term (B); the relations used in syllogisms are quantifiers. Fit into a formal syllogistic form, this argument would read:

\begin{enumerate}
\item \emph{All} officemates are out with the flu (All A are B)
\item \emph{Some} out with the flu are out for weeks (Some B are C) 
\item Therefore, \emph{some} officemates are out for weeks (Some A are C)
\end{enumerate}

Unfortunately, the conclusion does not follow from the premises.
...
Even for valid arguments, the accuracy of producing valid conclusions ranged from 90\% to 1\% \cite{Khemlani2012}: people do not seem to find drawing deductively valid conclusions particularly automatic.
