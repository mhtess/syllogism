...
In our modeling, we did not assume anything about these prior beliefs, except that they were constant across all syllogisms, and we used Bayesian data analysis to infer the prior beliefs used in the reasoner model, a kind of ``descriptive Bayesian'' approach \cite{Tauber2017}.
These prior beliefs form the basis for formalizing hypotheses about how content of the syllogism (i.e., the content of the terms: A, B, C) affects reasoning, a topic known that ``belief bias'' that has been of interest to psychologist for decades \mht{CITE}.
For example, \citeA{Tessler2015} found that content effects on reasoning could be captured by empirically measuring the relevant prior beliefs and plugging those into the Bayesian model. 
Further work should look to the prior beliefs to formalize hypotheses about how priors rationally affect reasoning, which could speak to outstanding debates about whether these affects are indeed a form of ``bias' ' \mht{CITE}. 
