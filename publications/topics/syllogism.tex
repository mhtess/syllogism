...
Imagine discussing a sports competition with your friend, and your friend remarks: 
...
You may conclude that probably some of the semifinalists had favorable referees.
The argument seems reasonable, and yet the conclusion is not necessarily true given the premises. (Consider that there could be teams that were expected to advance that did not make it to the semifinals and that those teams were the ones with favorable referees.)
If it is not logic that underlies our intuitions about what conclusions reasonably follow from an argument, then what does?

At least in the Western tradition, for as long as humans having been thinking about thinking, they have been thinking about logical arguments like the one above, known as syllogisms.
A syllogism is a formal logical argument that uses quantifiers (e.g. \emph{all}, \emph{some}) to relate two properties (e.g. being a \emph{semifinalist} and having \emph{favorable referees}) via a third (``middle'') property (e.g. being \emph{expected to advance}). 
...
Logic or syntactic-based approaches have attempted to articulate the ``natural'' logical rules that humans bring to bear when evaluating syllogistic arguments \cite{braine1983logical, rips1994, geurts2003reasoning}.
Semantic-based approaches have looked instead to diagram- or model-based representations of the situations implied by the premises and how these representations can be manipulated to arrive at a conclusion, the most prominent example being the Mental Models theory \cite{johnson1975models}.
...
Furthermore, a theory that integrates such a probabilistic logic with principles of natural language -- semantics and pragmatics -- which are intuitively at play for human reasoning \cite{sperber1986relevance,mercier2017enigma} has not been attempted.
Natural language semantics plays a role in many theories of reasoning \cite{JL1978, Khemlani2012, geurts2003reasoning} and have even been integrated with probabilities \cite{johnson2015logic}, but how those semantics interface with pragmatic reasoning  is either posited in an ad-hoc manner \cite{Chater1999} or has been limited to simple Gricean enrichment of the premises \cite{Roberts2001}.	
...
Here, we take up the task of developing a model of syllogistic reasoning that integrates logic, probability, and pragmatics in a rational way.
...
We developed a model of syllogism reasoning that uses the logic of the syllogism to update a probabilistically-generated distribution over world states and decides what pragmatically useful conclusion would best convey that distribution on states to a naive listener.
Below, we formalize each of these three ingredients -- probability, logic, and pragmatics -- and show how they combine to form the foundation of a rational model of syllogistic reasoning. 
...
A \emph{mental model} in the style of  \citeA{johnson1983mental} that is consistent with syllogism above would be:
...
We presented a Bayesian model that uses the literal, natural language semantics of the premises of the syllogism to updates its prior beliefs. 
It is straightforward to extend our approach to reasoning with other quantifiers (e.g., \emph{most} and \emph{few}; see e.g., \citeNP{tessler2014some}, cf., \citeNP{geurts2003reasoning}) as well as to consider the implications of pragmatics on the interpretation of the premises. 
...
We have only considered one particular experimental protocol and data set (from Ragni et al., 2019), where participants were presented with all 64 syllogisms, revealing the generative process of the syllogism (parametric recombination of quantifiers and term orders) and perhaps inducing a more logical analysis of the syllogisms.
