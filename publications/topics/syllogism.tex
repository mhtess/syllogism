...
Semantic approaches have theorized about the processes that manipulate  model- or diagram-based mental representations to arrive at a conclusion, the most prominent example being the Mental Models theory \cite{johnson1975models}.
Still other approaches attempt to articulate the functional and rational basis of   human logical reasoning, interrogating deductive reasoning as a special case of probabilistic inference \cite{oaksford2001probabilistic, oaksford2007bayesian, hahn2007rationality, tenenbaum2006theory}, which was formalized in the Probability Heuristics Model of syllogistic reasoning \cite{Chater1999}.


Though all this theoretical work provides insight into how humans reason with syllogisms, a model that unifies and explains syllogistic reasoning in terms of a rational computation is still outstanding.
...
The mental models approach provides a process-level account, recently enhanced with probabilistic constructs and assumptions \cite{johnson2015logic}, but not with the goal of providing a unified rational analysis. 
...
In other words, we assume a continuously valued semantics such that $\mathcal{L}(u, s) = 1-\phi$, a technique introduced by \citeA{degen2020redundancy}.
...
Our proposal is that human syllogistic reasoning is part logical, part probabilistic, and part pragmatic. Our theory is a rational model that articulates the functional problem human syllogistic reasoning is aiming to solve, which is fundamentally a problem of communication. Our work thus builds on the goal articulated by \citeA{Chater1999} of building a rational model of syllogistic reasoning, and we do so without positing any heuristic solution to the inference problems. Our model is deeply connected to the Mental Models approach \cite{johnson1975models, johnson2015logic} in that the states of affairs that our model reasons over are mental models of the world. Typically, a Mental Models theory would reason about tokens or instances of categories whereas our model reasons about types. 
