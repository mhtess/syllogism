...
Our modeling approach to pragmatic speakers also provides a clear, interpretable mechanism by which the \emph{nothing follows} option can be rationally produced, a shortcoming of most approaches to syllogistic reasoning  \cite{riesterer2020modeling}. 
In our formalization, \emph{nothing follows} is a semantically vacuous statement which does nothing to update the listener's beliefs. This kind of utterance is always true, in a sense, and thus speakers will produce this utterance with constant probability.\footnote{Technically, a literal speaker will produce this vacuous utterance with probability inversely proportional to the number of conclusions that are compatible (true) with the premises.}
...
Hence, when their beliefs are not substantially changed by hearing the premises (as is the case with logically invalid syllogisms), they will have more of a pressure to say \emph{nothing follows}, since it would also not substantially changed the listener's beliefs. 
...
We presented a Bayesian model that uses the natural language semantics of the premises of the syllogism to updates its prior beliefs about the relationship between three properties. 
These prior beliefs...


We presented a Bayesian model that uses the natural language semantics of the premises of the syllogism to updates its prior beliefs. It is straight-forward to consider the implications of pragmatics on the interpretation of the premises. 
Such a model could be constructed by a relatively standard Rational Speech Act pragmatic listener \cite{Frank2012a, goodman2016pragmatic, scontras2018probabilistic}; that is, treating the listener component of the model as a listener who reasons about why a speaker produced the utterances that they did, where the speaker is the kind of pragmatic speaker we have considered in this paper, who decides what utterance to produce in order to convey information about the state (or, belief distribution) to a naive literal listener. 
...
As well, the participants were \mht{confirm the participants of Ragni et al. were WEIRD?}.
Reasoning with syllogisms varies appreciably across individuals \mht{individual differences paper}, and substantially so in illiterate persons \mht{syllogisms and literacy paper},
