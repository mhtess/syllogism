...
We suggest that combining probabilistic reasoning with natural language semantics and pragmatics is a useful approach: an approach in which knowledge can describe distributions on possible situations, and in which these distributions can be updated by sentences with new information.
...
The pragmatic bayesian reasoner considers the space of all arguments. Many arguments in the syllogistic space are relatively vacuous, i.e. they do not update the prior substantially. As such, the most probable conclusion given all arguments is  \emph{not all X are Y}. Since the argument in [3] is more informative relative to others (e.g. the argument in [5]), the most likely intention of the experimenter is to convey that \emph{some A are C}.
