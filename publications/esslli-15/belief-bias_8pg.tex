...
In this analysis, I explore 4 different models of argument strength that differ in the details of their priors. The first model -- the ``abstract'' model -- uses a single base-rate parameter in its prior\footnote{This is the exact model of argument strength used by \citeA{Tessler2014}.); this model assumes properties are \emph{independent and identically distributed (i.i.d.)} both within domains and across domains. 
 ...
 First, I compare a model that uses the empirical priors to a model that uses a completely abstract prior: properties are \emph{i.i.d} within and across contexts. Second, I compare the empirical prior model to a model with context-sensitive but still independent priors on properties. 
