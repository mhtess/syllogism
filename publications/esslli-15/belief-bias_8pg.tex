...
In a preliminary experiment, I recruited 70 participants from Amazon's Mechanical Turk to judge the likelihood of various combinations of properties co-occurring (see Table \ref{tab:domains} for the properties used) \footnote{To increase reliability of the measure, I asked participants to rate either the \emph{frequency} (assuming some sample size) or the \emph{plausibility} of objects with properties. Measurements were highly reliable across dependent measures and so I collapsed across measures}. The resulting empirical priors from that preliminary experiment are shown in Figure \ref{fig:priors}. 

In this experiment,  I tested if syllogisms using the real world content from the preliminary experiment led to different conclusions.
...
For a first pass with data analysis, I analyzed only the radio buttons. Shown in Figure \ref{fig:syllogismXdomain} (darkest bars) are a subset of the results of the experiment. Content effects can be viewed by comparing panels within a row (compare across columns). For example, for the \emph{all / some} syllogism (top row), the proportion of ``all'' responses (red) in the knife domain was appreciably higher than in the strawberries domain. Effects of reasoning (i.e., of syllogism) can be observed by comparing panels within a column (compare across rows). For example, for the crackers domain (first column), the distribution over conclusions is appreciably different for the three syllogisms shown. Thus, I observe both content and reasoning effects using these materials.
...
Base rate parameters had a uniform prior distribution over them ($br \thicksim U(0,1)$). Inference was done via the Metropolis-Hastings algorithm implemented in the probabilistic programming language WebPPL \cite{dippl}. Models were analyzed independently, and MCMC chains were run for 10,000 samples. The posterior value for $\phi$, the response noise variable, was around 0.25 for all models; this analysis contributes about a quarter of the responses to noise (as opposed to the posited cognitive models). 
...
\multicolumn{5}{ |c| }{Experiment 1 Domains} \\
\hline
Model & Free parameters for prior & Other free parameters & Correlation with data  \\ \hline
%\multirow{4}{*}{Defenders} & LB & Lucus Radebe \\
abstract & 1 & 1 & 0.81  \\ \hline
iid & 4 & 1 & 0.86  \\ \hline
independent & 12 & 1 & 0.90 \\ \hline
empirical & 0 & 1 & 0.87  \\ \hline
\end{tabular}
