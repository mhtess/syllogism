...
The Bayesian model has a very natural way to account for prior beliefs in reasoning. Indeed, beliefs simply specify a different prior distribution of properties. In particular, the assumption that properties are \emph{independent and identically distributed (i.i.d.)} must be relaxed if we are to consider real-world content. I extend the model by considering that properties can that can have correlations ; the presence or absence of properties can be represented by the joint distribution: P$(A, B, C)$.
...
For syllogistic reasoning, we are interested not in the posterior distribution over situations \emph{per se}, but the distribution on true conclusions that these situations entail: $P(conclusion \mid s)$, where $conclusion$ is a quantifier-utterance relating the conclusion terms (e.g. $u_{\textrm{some  \emph{A} are \emph{C}}}$). Hence,

$$
P(conclusion \mid premises) \propto P(s \mid premises)\cdot P(conclusion\mid s)
$$
...
TG was able to capture this asymmetry by extending this model of argument strength to include pragmatic reasoning. 
