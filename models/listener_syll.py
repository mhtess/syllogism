...
def syllogism_model(n_balls,base_rate,qdepth,serv,nvc):
    ...
    ### RUNS CHURCH
    def run_church(rtnQ,fnl):
        prefix = 'EP%d_alphQ%.1f_n%d_base%.2f_s%dk' % (EP,rtnQ,n_balls,base_rate,n_samples)
        fname = 'listener_N' + ndepth + '_' + prefix + '.church'
        if (serv==1):
            os.chdir('/home/mht/webchurch/')
        else:
            os.chdir('/Users/mht/webchurch/')
        rname = 'listener_N' + ndepth + '_' + prefix + '.results'
    ...
    for rationalityQ in rationQrange:
        final = np.zeros((64,9))
        final = run_church(rationalityQ,final)
    ...
    if not os.path.exists((destination+'church/')):
        os.mkdir((destination+'church/'))
    ...
    for data in glob.glob((destination+'*.results')):
        ...
        os.remove(data)
    for data in glob.glob((destination+'*.church')):
        shutil.copy(data,(destination+'church/'))
        os.remove(data)
