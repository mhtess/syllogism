...
all_plots_prag <- map(unique(wp.rs.samples$utterance), 
                 function(x){ make_venn_digram (
                   wp.rs.samples %>% filter(src == "pragmatic"), x 
                   ) } )

cowplot::plot_grid(
  cowplot::plot_grid(plotlist = all_plots, nrow = 1) + 
    theme(plot.margin = unit(c(1.25,0,1,0), "lines")),
  cowplot::plot_grid(plotlist = all_plots_prag, nrow = 1)+ 
    theme(plot.margin = unit(c(1,0,0,0), "lines")),
  nrow = 2, labels = c("literal", "pragmatic"),
  label_size = 12,
  label_x = 0, #label_y = 0,
