...
for nobj in levels(models.c$variable){
  for dom in levels(models.c$domain){
    for sl in levels(models.c$syll){
      ...
      model.preds = subset(models.c, domain==dom) #subset by domain
      data.obs = subset(df.c, domain==paste(dom,"s",sep="") & condition=='radio')[,c("subj","syll","radio_A","radio_E","radio_I","radio_O")] # subset data by domain and condition==radio and take only radio button vals
      ...
      tot.obj <- length(levels(model.preds$variable))
...
data <- list("tot.subj", "tot.syll", "tot.resp",
             "emprior","depth","d","prior_on_objects",
             "churchmod") # to be passed on to JAGS

myinits <-  list(
  list(phi = 0.25, 
       x = array(round(runif(tot.subj*tot.syll*tot.resp)),
