...
(define (questioner conclusion Q_depth R_depth)
  (query 
    ...
    (equal? conclusion (reasoner premise-one premise-two Q_depth R_depth))))
...
(define (reasoner premise-one premise-two Q_depth R_depth)
  (query 
    ...
    (and (if (= N_depth 0)
    		 (and (premise-one A B)
             (premise-two B C))
      		 (equal? (list premise-one premise-two) (questioner conclusion (- Q_depth 1) R_depth)))
          (if (= M_depth 0)
              (conclusion A C)
              (equal? (list premise-one premise-two) (questioner conclusion Q_depth (- 1 R_depth)))))))

\end{lstlisting}
The \lstinline{reasoner} and \lstinline{questioner} functions produce a utility distribution over conclusions and premises, respectively. Conclusions or premises are then drawn from these distributions according to a softmax decision rule that describes an approximately rational planner \cite{suttonbarto98}. \lstinline{R_depth} and \lstinline{Q_depth} refer to the depth of recursion for each of these conditioning statements. As \lstinline{R_depth} increases, the conclusion becomes more informative. As \lstinline{Q_depth} increases, the premises becomes more informative. When both are set to 0, the model collapses to produce the $\Pr$(conclusion $\arrowvert$ premises). We refer to this model as the literal reasoner going forward. 
