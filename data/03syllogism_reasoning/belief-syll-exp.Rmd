...
%>%
  rename(half1 = 'TRUE', half2='FALSE')


%>%
  mutate(samp = 1)

table(test$syll,test$domain)
table(d.tidy$syll,d.tidy$domain)


## NOTE: NEED TO SAMPLE WITHIN EACH CONDITION
half1<-sample(subjects, n.subj/2)
half2<-subjects[!(subjects%in%half1)]

# pair up subjects, by just following the random order in which half1 was sampled
id<-match(d.tidy$subj,half1)
id2<-match(d.tidy$subj,half2)
id[is.na(id)] = id2[!is.na(id2)]

d.tidy$id<-factor(id)

d.tidy.split <- d.tidy %>%
  mutate(half = 1*subj%in%half1) %>%
  gather(conclusion,response,Q_A,Q_E,Q_I,Q_O) %>%
  group_by(conclusion,syll, domain) %>%
