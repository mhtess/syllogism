...
df.acq<-melt(subset(df.c,substring(condition,1,5)=='radio')[,c('experiment','acqAO','acqEI')])


df.acqs<-ddply(df.acq, .(experiment, variable), summarise, prop=sum(1-value)/length(value))

ggplot(df.acqs,aes(x=variable,y=prop,fill=experiment))+
...
ggplot(radios.unnorm,aes(x=value,y=justrad,color=variable))+
  geom_point(size=2.5)+
  theme_bw()
```
