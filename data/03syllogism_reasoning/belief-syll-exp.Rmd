...
dm.sub<-filter(dm.tidy, syllog%in%c("AI1","IE1","OA1"))
dm.sub$syllog<-factor(dm.sub$syllog, levels =c("AI1","IE1","OA1"), 
       labels = c("all / some", "some / none", "not all / all"))
...
ggplot(dm.sub, 
       aes(x=type, y = value, fill = conclusion, alpha = type, group=conclusion))+
  geom_bar(stat='identity', position=position_dodge(), color='black')+
  theme_paper()+
  scale_alpha_manual(values=c(0.3,0.7,1))+
  theme(axis.text.x = element_blank(),
