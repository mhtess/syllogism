...
fpath = 'C:/Users/Henry/Documents/RESEARCH/syllogism-project/data/03syllogism_reasoning/'


fpath2 = 'C:/Users/Henry/Documents/RESEARCH/syllogism-project/data/04syllogism_reasoning/'
...
# 2a. Joint or marginals? / Splift-half & prophecy

How much information is there in the joint distributions of the subjects, relative to the marginal distributions? Another way of saying this is "how important is subject ID"? One way to (crudely) look at this is to take the split half correlation with permuting subject ID and compare it with the split half correlation without permuting.
...
d.tidy<-df.c %>% 
  select(subj,Q_A,Q_E,Q_I,Q_O,condition,syll,domain, experiment) %>%
  filter(condition%in%c("radio","radio.2"),experiment=='1') %>%
...
d.z<-d.tidy %>% 
  gather(conclusion, value, starts_with("Q_")) %>%
  group_by(subj) %>%
  mutate(val_z= (value-mean(value))/sqrt(var(value))) %>%
  ungroup() %>%
  select(-value) %>%
  spread(conclusion,val_z)
...
d.resamp<-d.z %>%
  group_by(syll,domain) %>%
  sample_frac(size=1, replace=F) %>%
  mutate(count = cumsum(to.n(experiment))) %>%
  mutate(half = ifelse(count < (length(experiment)/2), 'half1','half2')) %>%
  gather(conclusion, value,Q_A,Q_E,Q_I,Q_O) %>%
  group_by(syll,domain,half, conclusion) %>%
...
# Bayesian analysis of Prior elicitation data


```{r fbt.priors}
d<-read.csv('lightbulbposterior_test.csv')

d.tidy<-d %>% 
  gather(parameter, value, -prob) %>%
  group_by(parameter, value) %>%
  summarise(marginal = sum(prob))

ggplot(data=filter(d.tidy,parameter=='alpha'), aes(x=value,y=marginal))+
...
# Analysis of radio buttons alone

```{r}
fpath = '~/Documents/research/syllogism/data/03syllogism_reasoning/'
fpath2 = '~/Documents/research/syllogism/data/04syllogism_reasoning/'
#fpath = 'C:/Users/Henry/Documents/RESEARCH/syllogism-project/data/03syllogism_reasoning/'
#fpath2 = 'C:/Users/Henry/Documents/RESEARCH/syllogism-project/data/04syllogism_reasoning/'
...
df.tidy<- df.c %>% 
  ...
  group_by(domain, syll, quantifier) %>%
  summarise(prop = sum(response)/length(response),
            n = length(response),
            sterr = sqrt(((sum(response)/length(response))*
                    (1-(sum(response)/length(response))))/(length(response))))



ggplot(df.tidy, aes(x=quantifier, y = prop, fill = quantifier))+
  geom_bar(stat='identity', position=position_dodge())+
  geom_errorbar(aes(ymin=prop-2*sterr, ymax = prop+2*sterr), 
...
df.tidy<-df.tidy %>% rename(conclusion = quantifier, 
                   syllog = syll) %>% ungroup()

df.tidy$conclusion <- factor(df.tidy$conclusion, 
...
#####
ggplot(df.norm, aes(x=conclusion, y = prop, fill = conclusion))+
  geom_bar(stat='identity', position=position_dodge())+
  geom_errorbar(aes(ymin=prop-2*sterr, ymax = prop+2*sterr), 
                position=position_dodge(),
                width = 0.3, size = 0.3)+
...
# Experimental data: Using radio buttons only

## Experiment 1, just radio buttons
```{r exp1.radio}
head(df)

d1.tidy<- df %>%
  filter(condition=='radio') %>%
  select(subj, starts_with('radio'), syll, domain) %>%
  rename(all=radio_A,
         none = radio_E,
         some = radio_I,
         not.all = radio_O,
         syllog=syll) %>%
  gather(conclusion, response, all, none, some, not.all)

bootstrap.radio <- function(x){
  agr = aggregate(response ~ domain + conclusion + syllog, data=x, FUN=mean)
  agr$CILow = aggregate(response ~ domain + conclusion + syllog, data=x, FUN=ci.low)$response
  agr$CIHigh = aggregate(response ~ domain + conclusion + syllog, data=x, FUN=ci.high)$response
  agr$YMin = agr$response - agr$CILow
  agr$YMax = agr$response + agr$CIHigh
  return(agr)
}

d1.btstr<-bootstrap.radio(d1.tidy)

ggplot(d1.btstr, aes(x=conclusion, y = response, fill = conclusion))+
...
## Experiment 2, just radio buttons
...
d.btstr<-bootstrap.radio(d.tidy)

ggplot(d.btstr, aes(x=conclusion, y = response, fill = conclusion))+
  geom_bar(stat='identity')+
  geom_errorbar(aes(ymin = YMin, ymax = YMax), 
                width = 0.3)+
  facet_grid(domain~syllog)
...
# Full Bayesian Treatment using WPPL

## Experiment 1
```{r}
setwd("~/Documents/research/syllogism/analysis")
d<-read.csv('../models/FBT/exp1_n5Marginal_posteriorD_wPrior_test_100.csv')
d<-read.csv('../models/FBT/exp1_n4Marginal_posteriorD_backoffUnif_test_100.csv')
d<-read.csv('../models/FBT/exp1_n4Marginal_posteriorD_backoffBinom_test_100.csv')
d<-read.csv('../models/FBT/exp1_n4Marginal_posteriorD_wonky_test_1000.csv')




params<-d %>%
  select(phi, alpha, prob) %>%
  gather(param, val, -prob) %>%
  group_by(param,val) %>%
  summarise(marginal = sum(prob))

ggplot(params, aes(x=val, y= marginal))+
...
predict$conclusion<-factor(predict$conclusion, levels = c('all','none','some','not.all'))
predict$syllog<-factor(predict$syllog)
predict$domain<-factor(predict$domain, labels = c("crackers","knives","lightbulbs","strawberries"))

md<-left_join(d1.btstr, predict, by=c("syllog","domain","conclusion"))
md.tidy<- md %>%
  gather(type, value, response, expval)

md.tidy$type<-factor(md.tidy$type, levels=c("response","expval"), labels=c("data","model"))





ggplot(predict, aes(x=conclusion, y= expval, fill=conclusion))+
  geom_bar(stat='identity')+
  facet_grid(domain~syllog)



with(md, cor(response, expval))

ggplot(data=md, aes(x=expval, y = response, color=conclusion))+
  geom_point()

ggplot(data=md, aes(x=expval, y = response, color=syllog))+
  geom_point()

ggplot(data=md, aes(x=expval, y = response, color=domain))+
  geom_point()

ggplot(md.tidy, aes(x=conclusion, y= value, fill=type))+
  geom_bar(stat='identity', position=position_dodge())+
    #geom_errorbar(aes(ymin = YMin, ymax = YMax), 
...
### Model and data analysis parameters

```{r}
setwd("~/Documents/research/syllogism/analysis")
d<-read.csv('../models/FBT/exp2_n4Marginal_posteriorD_wPrior_test_1000.csv')
d<-read.csv('../models/FBT/exp2_n4Marginal_posteriorD_backoffUnif_test_100.csv')
d<-read.csv('../models/FBT/exp2_n4Marginal_posteriorD_backoffBinom_test_100.csv')


params<-d %>%
 # select(phi, alpha, prob) %>%
  #select(phi, br, prob) %>%
  select(phi, starts_with('br'), prob) %>%
  select(phi,prob) %>%
  gather(param, val, -prob) %>%
  group_by(param,val) %>%
  summarise(marginal = sum(prob))
...
brs <- d %>%
  select(starts_with("lightbulb"), starts_with("cracker"),starts_with("knife"),
         starts_with("strawberry"), prob) %>%
  gather(param, val, -prob) %>%
  separate(param, into=c("domain","property"), by="_") %>%
  group_by(domain, property, val) %>%
  summarise(marginal = sum(prob))

ggplot(brs, aes(x = val, y = marginal, fill = domain))+
  geom_histogram(stat='identity', position=position_dodge())+
  facet_grid(domain ~ property)
...
predict<-d %>% 
  #select(-phi,-alpha) %>%
  select(-phi, -starts_with("lightbulb"), -starts_with("cracker"),
         -starts_with("knife"),
         -starts_with("strawberry")) %>%
#  select(-phi, -br) %>%
select(-phi) %>%
#  select(-phi, -starts_with('br')) %>%
  gather(item, val, -prob) %>%
  separate(item, into=c("syllog","domain","conclusion"), sep="_") %>%
  group_by(syllog, domain, conclusion) %>%
  summarise(expval = sum(prob*val))
...
predict$syllog<-factor(predict$syllog)
predict$domain<-factor(predict$domain, labels = c("crackers","knives","lightbulbs","strawberries"))
md<-left_join(d.btstr, predict, by=c("syllog","domain","conclusion"))
md.tidy<- md %>%
  gather(type, value, response, expval)
...
```{r fbt.exp1.exp2.params}
setwd("~/Documents/research/syllogism/analysis")
d<-read.csv('../models/FBT//exp12_n4Marginal_wonky_1000a_combined.csv')

d<-read.csv('../models/FBT/exp12_n4Marginal_r0_empirical_10000a_combined.csv')



params<-d %>%
  select(phi, alpha, priorwonky,prob) %>%
  #select(phi, alpha,prob) %>%
  gather(param, val, -prob) %>%
  group_by(param,val) %>%
  summarise(marginal = sum(prob))
...
predict$conclusion<-factor(predict$conclusion, 
                           levels = c('all','none','some','not.all'))
predict$syllog<-factor(predict$syllog)
predict$domain<-factor(predict$domain, 
                       labels = c("crackers","knives","lightbulbs","strawberries"))



d12.btstr<-bind_rows(d.btstr,d1.btstr)
d12.btstr$syllog<-factor(d12.btstr$syllog)

## normalize so responses add to 1
dbt.norm<-d12.btstr %>%
  group_by(domain, syllog) %>%
...
md<-left_join(dbt.norm, predict, by=c("syllog","domain","conclusion"))
md.tidy<- md %>%
  gather(type, value, response, expval)
...
# Bayesian model comparison: Wonky RSA vs. Regular RSA

### Model and data analysis parameters

```{r fbt.exp1.exp2.params}
setwd("~/Documents/research/syllogism/analysis")
#d<-read.csv('../models/FBT/modelComparison_wRSAvsrRSA_n4_10000c_combined.csv')

d<-read.csv('../models/FBT/modelComparison_allPrems_wRSAvsrRSA_n4_10000b_combined.csv')

#ggplot(d, aes(x= wonkyHypothesis, y = wonkyprior))+geom_point()
...
#dm<-left_join(df.norm, predict, by=c("syllog","domain","conclusion"))
# to get bootstrapped 95% CI in plot
dm<-left_join(d.bs.t, predict, by=c("syllog","domain","conclusion"))


with(dm, cor(response, expval))
# 
# empirical<-predict
# independent<-predict
# 
# independent<- independent %>% rename(independent = expval)
# empirical <- empirical %>% rename(empirical = expval)
# 
# models <- left_join(independent, 
#                     empirical, by=c("syllog","domain","conclusion"))
# 
# dm<-left_join(df.norm, models, by=c("syllog","domain","conclusion"))
# 
# 
# dm$conclusion<- factor(dm$conclusion, labels = c("all", "none", "some", "not all"))

with(dm, cor(prop, independent))
with(dm, cor(prop, empirical))
with(dm, cor(empirical, independent))

ggplot(dm, aes(x=empirical, y=prop, color=conclusion))+
  geom_point(size=2)+
  theme_paper()+
  xlim(0,0.6)+
  ylim(0,0.6)+
  scale_color_brewer(type='qual',palette=6)+
  xlab("(empirical prior) model predictions")+
  ylab("human proportion endorsements")+
  coord_fixed()

dm.tidy<- dm %>% 
#  gather(type, value, prop, empirical, independent)
  gather(type, value, response, expval)

# dm.tidy$type <- factor(dm.tidy$type, levels=c("independent","empirical", "prop"),
#                        labels=c("(independent) model", "(empirical) model", "human data"))
dm.tidy$type <- factor(dm.tidy$type, levels=c("response", "expval"),
                       labels=c( "human data","(empirical prior) model"))



dm.sub<-filter(dm.tidy, syllog%in%c("AI1","IE1","AA1"))
dm.sub$syllog<-factor(dm.sub$syllog, levels =c("AI1","IE1","AA1"), 
       labels = c("all / some", "some / none","all / all"))
dm.sub$conclusion<-factor(dm.sub$conclusion, 
...
ggsave(p, filename='../publications/esslli-15/figures/bars_bw.pdf',
       height = 5.5,
       width=11)


ggplot(dm.sub, 
       aes(x=conclusion, y = value, fill = type))+
  geom_bar(stat='identity', 
           position=position_dodge(), color='black')+
  theme_paper()+
...
#  scale_fill_brewer(type='qual',palette=2)+
  #geom_errorbar(aes(ymin=value-2*sterr, 
  #                  ymax = value+2*sterr), 
  #              position=position_dodge(),
  #              width = 0.3, size = 0.3)+
  facet_grid(syllog~domain)+
